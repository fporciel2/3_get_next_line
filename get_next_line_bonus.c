...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/28 08:53:40 by fporciel          #+#    #+#             */
/*   Updated: 2023/03/28 09:41:13 by fporciel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
/* 
...
char	*get_next_line(int fd)
{
	...
	if ((fd < 0) || (fd > 4096) || (BUFFER_SIZE <= 0))
		return (NULL);
	size = get_length(buf[fd], 0);
	count = 1;
	while ((!(get_length(buf[fd], 1))) && (count > 0))
	{
		buf[fd] = get_realloc(&(buf[fd]), 0);
		if (buf[fd] == NULL)
			return (NULL);
		count = read(fd, &(buf[fd][size]), BUFFER_SIZE);
		if (count < 0)
			return (ft_free_null(&(buf[fd]), NULL, NULL));
		size = (size + count);
		if (count != 0)
			buf[fd][size] = 0;
	...
	if ((count < BUFFER_SIZE) || (count == 0))
		buf[fd] = get_realloc(&(buf[fd]), 1);
	return (get_return(&(buf[fd])));
}
